...
/**
 * Demonstration program to run the Tempura optimizer. This program produces a progressive physical
 * plan by the Tempura optimizer that runs across several time points. The physical plan is printed
 * out to the console in DOT format, which can be viewed using an online graphviz tool.
 */
public class TvrOptimizationTest {
  ...
  // there will be 2 early runs that produce partial results, and 1 final run that produces the
  // final result
  //
  public static String instants = "28800000, 57600000";

  @Test public void testSumAgg() throws Exception {
    String query = "select deptno, sum(salary) from \"hr\".\"emps\" group by deptno\n";
    testQuery(query);
  }

  @Test public void testSumCountAgg() throws Exception {
    String query =
        "select deptno, sum(salary), count(salary) from \"hr\".\"emps\" group by deptno\n";
    testQuery(query);
  }
  ...
  @Test public void testJoin() throws Exception {
    String query =
        "select *n" + "from hr.emps as en" + "join hr.depts as dn"
  ...
  public void testQuery(String query) throws Exception {
    ...
    Program hepProgram = Programs.hep(ImmutableList
            .of(JoinProjectTransposeRule.RIGHT_PROJECT, JoinProjectTransposeRule.LEFT_PROJECT,
                FilterJoinRule.FilterIntoJoinRule.FILTER_ON_JOIN, ProjectRemoveRule.INSTANCE,
                ProjectMergeRule.INSTANCE, JoinToMultiJoinRule.INSTANCE), false,
        DefaultRelMetadataProvider.INSTANCE);
    Program tvrProgram =
        new Programs.TvrRuleSetProgram(RuleSets.ofList(TvrRuleCollection.tvrStandardRuleSet()));
    ...
    final FrameworkConfig config =
        Frameworks.newConfigBuilder().parserConfig(SqlParser.Config.DEFAULT)
            ...
            .traitDefs(ConventionTraitDef.INSTANCE
                //                        ,RelCollationTraitDef.INSTANCE, RelDistributionTraitDef
                //                        .INSTANCE
            )
            //                .programs(Programs.standard())
            .programs(hepProgram, tvrProgram).context(Contexts.of(tvrContext)).build();
    ...
    RelNode volcanoOutput = planner
        .transform(1, rel.getCluster().traitSet().replace(EnumerableConvention.INSTANCE),
            hepOutput);
