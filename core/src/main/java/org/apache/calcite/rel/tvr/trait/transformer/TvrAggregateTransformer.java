...
public class TvrAggregateTransformer extends TvrSemanticsTransformer {
  ...
  /**
   * Bit set of grouping fields. The indices referring to the input schema.
   */
  ...
  /**
   * index of the multiplicity column with type BIGINT, it is required in positive/negative delta
   * where +1 represents insertion and -1 represents deletion multiplicity is a part of group set
   */
  ...
  public TvrAggregateTransformer(ImmutableBitSet groupSet,
      ImmutableList<AggregateCall> aggregateCalls, ImmutableSet<Integer> primaryKeys,
    ...
    for (int i = 0; i < aggregateCalls.size(); i++) {
      ImmutableIntList args = ImmutableIntList.copyOf(aggregateCalls.get(i).getArgList());
  ...
  @Override public TvrSemanticsTransformer transform(Multimap<Integer, Integer> mapping) {
    ...
    List<Integer> newGroupSet = groupSet.asList().stream().filter(mapping::containsKey).map(i ->
        // the mapping must contain the original group column
        Objects.requireNonNull(getFirstMappingResult(mapping, i))).collect(Collectors.toList());
    ...
    List<Integer> newGroupCols =
        mapping.entries().stream().filter(e -> e.getKey() < 0).map(Map.Entry::getValue)
    ...
    // 3. update aggCalls
    List<AggregateCall> newAggregateCalls = aggregateCalls.stream()
        .filter(aggCall -> aggCall.getArgList().stream().allMatch(mapping::containsKey))
        .map(oldCall -> {
          List<Integer> newArgList =
              oldCall.getArgList().stream().map(i -> mapping.get(i).iterator().next())
                  .collect(Collectors.toList());

          return AggregateCall
              .create(oldCall.getAggregation(), oldCall.isDistinct(), oldCall.isApproximate(),
                  false, newArgList, oldCall.filterArg, RelCollations.EMPTY, groupSet.size(), null,
                  oldCall.type, oldCall.name);
        }).collect(Collectors.toList());

    return new TvrAggregateTransformer(ImmutableBitSet.of(newGroupSet),
        ImmutableList.copyOf(newAggregateCalls), ImmutableSet.copyOf(newPrimaryKeys),
        multiplicityIndex);
  }
  ...
  /**
   * Check if the given index in the SetSnapshot schema refers to an aggregate column that is
   * partially computed in this value semantics
   */
  ...
  @Override public List<RelNode> apply(List<RelNode> inputs) {
    ...
    List<Pair<List<Integer>, List<Integer>>> resolveMap = this.aggregateCalls.stream()
    ...
    RexNode greaterThanZero = rexBuilder
        .makeCall(SqlStdOperatorTable.GREATER_THAN, ImmutableList.of(countInputRef, zeroLiteral));

    // create filter
    return finalAggregates.stream().map(agg -> LogicalFilter.create(agg, greaterThanZero))
        .collect(Collectors.toList());
  }
    ...
    aggregateCalls.forEach(aggCall -> {
      ...
      if (aggFunc.equalsIgnoreCase("MIN") || aggFunc.equalsIgnoreCase("MAX")) {
        ...
        if (aggCall.getType().isNullable()) {
          finalAggregateCalls.add(AggregateCall
              .create(SUM, aggCall.isDistinct(), aggCall.isApproximate(), false,
                  aggCall.getArgList(), aggCall.filterArg, RelCollations.EMPTY, groupCount, input,
                  null, aggCall.name));
        } else {
          finalAggregateCalls.add(AggregateCall
              .create(SUM0, aggCall.isDistinct(), aggCall.isApproximate(), false,
                  aggCall.getArgList(), aggCall.filterArg, RelCollations.EMPTY, groupCount, input,
                  null, aggCall.name));
        }
      } else {
        throw new UnsupportedOperationException(
            "unsupported agg function for final aggregate " + aggCall.name);
      }
    });

    return LogicalAggregate.create(input, ImmutableList.of(), groupSet, null, finalAggregateCalls);
  }
  ...
  @Override public RelDataType deriveRowType(RelDataType inputRowType, RelDataTypeFactory factory) {
  ...
  @Override public TvrSemanticsTransformer addNewTransformer(
    ...
    if (newTransformer instanceof TvrCompositeTransformer) {
      ...
      transformers.addAll(((TvrCompositeTransformer) newTransformer).getInnerTransformers());
      return new TvrCompositeTransformer(transformers);
    }
    return new TvrCompositeTransformer(Lists.newArrayList(this, newTransformer));
  }
  ...
  /**
   * Return the Count(1) AggregateCall added by TVR value semantics. We follow the convention that
  ...
  public int getIndexOfCountInOutputSchema() {
    return forwardMapping.get(ImmutableIntList.copyOf(getCountAggregateCall().getArgList()))
        .iterator().next();
  }
  ...
  public static class TvrAggregateTransformerSerde
      implements JsonDeserializer<TvrAggregateTransformer>, JsonSerializer<TvrAggregateTransformer>,
      TvrJsonUtils.ColumnOrderAgnostic {

    protected RelDataType rowType;
    protected RelDataTypeFactory typeFactory;

    public TvrAggregateTransformerSerde(RelDataType rowType, RelDataTypeFactory typeFactory) {
      this.rowType = rowType;
      this.typeFactory = typeFactory;
    }

    @Override public TvrAggregateTransformer deserialize(JsonElement json, Type typeOfT,
        JsonDeserializationContext context) throws JsonParseException {
      JsonObject jsonObject = json.getAsJsonObject();
      ImmutableBitSet groupSet = ImmutableBitSet
          .of(c2i(context.deserialize(jsonObject.get("groupSet"), String[].class), rowType));
      ImmutableSet<Integer> primaryKeys = ImmutableSet
          .copyOf(c2i(context.deserialize(jsonObject.get("primaryKeys"), String[].class), rowType));

      List<AggregateCall> aggCalls = new ArrayList<>();
      jsonObject.getAsJsonArray("aggregateCalls").forEach(
          je -> aggCalls.add(context.deserialize(je.getAsJsonObject(), AggregateCall.class)));
      ImmutableList<AggregateCall> aggregateCalls = ImmutableList.copyOf(aggCalls);

      Integer multiplicity = null;
      if (jsonObject.get("multiplicity") != null) {
        multiplicity =
            rowType.getFieldNames().indexOf(jsonObject.get("multiplicity").getAsString());
      }

      return new TvrAggregateTransformer(groupSet, aggregateCalls, primaryKeys, multiplicity);
    }

    @Override public JsonElement serialize(TvrAggregateTransformer src, Type typeOfSrc,
        JsonSerializationContext context) {
      JsonObject jsonObject = new JsonObject();

      jsonObject.add("groupSet", context.serialize(i2c(src.groupSet.asList(), rowType)));
      jsonObject.add("primaryKeys", context.serialize(i2c(src.primaryKeys.asList(), rowType)));
      if (src.multiplicityIndex != null) {
        jsonObject.add("multiplicity",
            context.serialize(rowType.getFieldList().get(src.multiplicityIndex).getName()));
      }
