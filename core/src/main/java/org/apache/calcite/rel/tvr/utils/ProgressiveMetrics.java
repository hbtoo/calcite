...
public class ProgressiveMetrics {
  ...
  public String summarize(Config optimizerConfig) {
    String queryName = optimizerConfig.get(TvrUtils.PROGRESSIVE_METRICS_QUERY_NAME, "query");
    ...
    phaseTime.forEach(
        (k, v) -> builder.append(k.toString()).append("=").append(v.toString()).append("\t"));
    builder.append("\n");

    relNodeCount
        .forEach((clazz, count) -> System.out.println(clazz.getSimpleName() + ": " + count));
    System.out.println("\nJoinCount: " + joinCount);
    System.out.println("AggregateCount: " + aggCount);
    System.out.println("Progressive Metrics summary:n" + builder.toString());

    String summary = builder.toString();
    if (optimizerConfig.getBool(TvrUtils.PROGRESSIVE_METRICS_OUTPUT_ENABLE, false)) {
      Path targetPath = Paths.get(TvrUtils.getProgressiveMetricsOutputPath(optimizerConfig));

      try {
        FileOutputStream timerOut;
        if (!targetPath.toFile().exists()) {
          Files.write(targetPath, "".getBytes(Charset.defaultCharset()));
        }
        timerOut = new FileOutputStream(targetPath.toString(), true);
        timerOut.write(summary.getBytes(Charset.defaultCharset()));
        timerOut.close();
      } catch (IOException e) {
        LOG.error("Fail to write metrics to file " + targetPath, e);
      }
    }
