...
public class TvrTableScanRule extends RelOptTvrRule {
  public static final TvrTableScanRule INSTANCE = new TvrTableScanRule(false, false);
  public static final TvrTableScanRule SAMPLE_INSTANCE = new TvrTableScanRule(true, false);
  public static final TvrTableScanRule COPY_INSTANCE = new TvrTableScanRule(false, true);
  ...
  private TvrTableScanRule(boolean sample, boolean copy) {
    super(
        operand(LogicalTableScan.class, s -> !TvrTableUtils.isDerivedTable(s),
  ...
  public static RelNode createTableScan(RelOptTable table, TimeInterval inv, RelBuilder relBuilder,
    ...
    if (table instanceof RelOptTableImpl) {
      ...
      if (relOptTable.getTable() instanceof ReflectiveSchema.FieldTable) {
        ReflectiveSchema.FieldTable<?> fieldTable =
            (ReflectiveSchema.FieldTable<?>) ((RelOptTableImpl) table).getTable();
        ...
        Enumerable subEnumerable =
            Linq4j.asEnumerable(enumerable.toList().subList(subListIndex.left, subListIndex.right));
        //        Enumerable subEnumerable = enumerable.skip(subListIndex.left).take(subListIndex
        //        .right - subListIndex.left);

        ReflectiveSchema.FieldTable<?> newFieldTable =
            new ReflectiveSchema.FieldTable<>(fieldTable.getField(), fieldTable.getElementType(),
                subEnumerable, fieldTable.getStatistic());
        ...
        Double newRowCount = relOptTable.getRowCount() * ((subListIndex.right - subListIndex.left)
        ...
        relOptTable = RelOptTableImpl.create(rootSchema, rowType, newFieldTable,
            ImmutableList.of(relOptTable.getQualifiedName().get(0), tableName), clazz -> Schemas
                .tableExpression(schemaPlus, newFieldTable.getElementType(), tableName, clazz),
            newRowCount, inv);
      ...
      throw new UnsupportedOperationException(
          "internal table of type " + relOptTable.getTable().getClass() + " is not supported yet");
    }
    throw new UnsupportedOperationException(
        "table of type " + table.getClass() + " is not supported yet");
  }

  public static Pair<Integer, Integer> getSubListIndex(int size, TimeInterval inv,
      RelOptCluster cluster) {
    TvrContext ctx = TvrContext.getInstance(cluster);
    long[] tvrVersions = ctx.getDefaultTvrType().getTvrVersions();
    int versionFrom =
        inv.from == TvrVersion.MIN_TIME ? 0 : Arrays.binarySearch(tvrVersions, inv.from) + 1;
    int versionTo =
        inv.to == TvrVersion.MIN_TIME ? 0 : Arrays.binarySearch(tvrVersions, inv.to) + 1;
