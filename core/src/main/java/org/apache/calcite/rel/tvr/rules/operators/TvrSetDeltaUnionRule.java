...
import static org.apache.calcite.rel.tvr.utils.TvrUtils.NOT_SET_SNAPSHOT;
...
public class TvrSetDeltaUnionRule extends RelOptTvrRule {
  ...
  private static RelOptRuleOperand operandOf(int numInputs) {
    Supplier<RelOptRuleOperand> leaf = () -> operand(RelSubset.class,
        tvrEdgeSSMax(
            tvr(tvrEdge(TvrSetDelta.class, NOT_SET_SNAPSHOT, logicalSubset())),
            IDENTICAL_TIME), any());
  ...
  @Override public boolean matches(RelOptRuleCall call) {
    ...
    return
        root.inputs().stream().map(i -> i.tvrSibling().tvrSemantics.fromVersion).distinct().count()
            == 1
            && root.inputs().stream().map(i -> i.tvrSibling().tvrSemantics.toVersion).distinct()
                .count() == 1;
  }
