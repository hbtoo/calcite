...
public class TvrSetSnapshotToSetDeltaRule extends TvrConverterRule {
  ...
  public static final TvrSetSnapshotToSetDeltaRule INSTANCE =
  ...
  private RelNode diffUsingAggregation(RelNode leftNode, RelNode rightNode, RexBuilder rexBuilder,
      TvrSetDelta setDelta, boolean consolidate) {
  ...
  private void diffUsingAntiJoin(
    ...
    RelNode leftAntiJoin = LogicalJoin
        .create(leftInput, rightInput, ImmutableList.of(), joinCondition, ImmutableSet.of(),
            JoinRelType.ANTI);

    RelNode rightAntiJoin = LogicalJoin
        .create(rightInput, leftInput, ImmutableList.of(), joinCondition, ImmutableSet.of(),
            JoinRelType.ANTI);
  ...
  private TvrConvertMatch create(TvrSemantics leftTrait,
    ...
    return new TvrConvertMatch(setDelta, ImmutableList.of(rel), true, tvrMetaSet ->
        tvrMetaSet.allTvrSemantics().stream().anyMatch(x ->
            (x instanceof TvrSetDelta || x instanceof TvrValueDelta && !((TvrValueDelta) x)
                .hasWindow()) && x.timeRangeEquals(setDelta)) || tvrMetaSet.allTvrSemantics()
            .stream().noneMatch(x -> x instanceof TvrValueDelta));
  }
}
