...
public class TvrContext {
  ...
  public long getCurrentOptimizationTime() {
    if (!TvrUtils.progressiveEnabled(this) || !TvrUtils.progressiveReoptimizeEnabled(this)) {
      return 0;
    }
      ...
      @Override public void visit(RelNode node, int ordinal, RelNode parent) {
        ...
        if (node instanceof LogicalTableScan) {
          ...
          updateTables.forEach(updateTable -> {
            ...
            } else if (nonDimTableOrdinals.putIfAbsent(updateTable, nonDimTableOrdinals.size())
                == null) {
  ...
  public void reloadUpstreamTablePropertyFromMeta() {
    ...
    //                            // get the derived tables and set the row count estimated by
    //                            upstream query
  ...
  public Map<String, TvrDataArrivalInfo> getTvrDataArrivalInfos() {
    if (tvrDataArrivalInfos == null) {
      String arrivalInfoJson = this.getConfig().get(PROGRESSIVE_EXP_DATA_ARRIVAL_INFO);
      if (arrivalInfoJson == null) {
        this.tvrDataArrivalInfos = Collections.emptyMap();
      } else {
        Gson gson = new GsonBuilder().setLenient().create();
        this.tvrDataArrivalInfos =
            gson.fromJson(arrivalInfoJson, new TypeToken<Map<String, TvrDataArrivalInfo>>() {
            }.getType());
      }
    }
  ...
  private long[] parseTvrVersions() {
    ...
    return LongStream.concat(
        Arrays.stream(invSetting.split(",")).map(String::trim).filter(s -> !s.isEmpty())
            .mapToLong(Long::parseLong), LongStream.of(TvrVersion.MAX_TIME)).sorted().toArray();
  }

  //    private long[] parseTvrVersionsByTablePartitions() {
  //        if (tvrRangeQueryPartitionManager == null) {
  //            throw new IllegalArgumentException("TvrRangeQueryPartitionManager does not
  //            initialize");
  //        }
  //
