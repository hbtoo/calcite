 ...
 *
 *
 * <p>    LogicalProject  --SET_SNAPSHOT_MAX-- Tvr0
 *           |                                     (SetDelta/ValueDelta)
...
public abstract class TvrProjectRules {
   ...
   * 1,  D,  D,      | \                     |                     |      /|  1, D, key_A, COUNT()
   *   AVG(C)        | |                      -------- M5 ---------      | |
   *                 | M1                                               M3 |
   *                 | |             --------------M2---------           | |
   *                 |/              |                        |           \|
   * Group by A,D   Aggregate  <-    *   <-  Transformer  <-  *  <--  Aggregate A,D,SUM(B),SUM(C),
   * SUM(B), AVG(C)   |                                                    |     COUNT(C),COUNT()
   *                  |                                                    |
...
class TvrValueSemanticsProjectRule extends TvrProjectRuleBase {
  ...
  private static TransformerPredicate inputRefPredicate =
      new TransformerPredicate() {
        ...
        @Override public boolean test(TvrProjectTransformer transformer,
            ImmutableIntList indices) {
          return indices.size() == 1;
        }

        @Override public boolean test(TvrSortTransformer transformer,
                            ImmutableIntList indices) {
          return indices.size() == 1;
        }
      };

  private static TransformerPredicate nonInputRefPredicate =
      new TransformerPredicate() {
        @Override public boolean test(TvrAggregateTransformer transformer,
            ImmutableIntList indices) {
          if (indices.size() != 1) {
        ...
        @Override public boolean test(TvrSortTransformer transformer,
          ...
          // 'rowNumberColIndex == null' means that row number column is not kept in the current
          // schema, and it has been removed by the previous projects.
  ...
  @Override public boolean matches(RelOptRuleCall call) {
    ...
    return logicalProject.getProjects().stream().allMatch(project -> {
      TransformerPredicate predicate =
          project instanceof RexInputRef ? inputRefPredicate : nonInputRefPredicate;
      return TvrRelOptUtils.findUniqueInputRef(project).stream()
          .allMatch(p -> transformer.isCompatible(predicate, ImmutableIntList.of(p.getIndex())));
    });
  }
