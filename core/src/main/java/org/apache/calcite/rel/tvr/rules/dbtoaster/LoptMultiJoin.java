...
/**
 * Utility class that keeps track of the join factors that make up a {@link MultiJoin}.
 */
public class LoptMultiJoin {
  ...
  /**
   * Join filters associated with the MultiJoin, decomposed into a list. Excludes left/right outer
   * join filters.
   */
  private List<RexNode> joinFilters;

  /**
   * All join filters associated with the MultiJoin, decomposed into a list. Includes left/right
   * outer join filters.
   */
  ...
  /**
   * If a join factor is null generating in a left or right outer join, joinTypes indicates the join
   * type corresponding to the factor. Otherwise, it is set to INNER.
   */
  private final ImmutableList<JoinRelType> joinTypes;

  /**
   * If a join factor is null generating in a left or right outer join, the bitmap contains the
   * non-null generating factors that the null generating factor is dependent upon
   */
  private final ImmutableBitSet[] outerJoinFactors;

  /**
   * Bitmap corresponding to the fields projected from each join factor, after row scan processing
   * has completed. This excludes fields referenced in join conditions, unless the field appears in
   * the final projection list.
   */
  private List<ImmutableBitSet> projFields;

  /**
   * Map containing reference counts of the fields referenced in join conditions for each join
   * factor. If a field is only required for a semijoin, then it is removed from the reference
   * count. (Hence the need for reference counts instead of simply a bitmap.) The map is indexed by
   * the factor number.
   */
  ...
  /**
   * Bitmap indicating which factors each factor references in join filters that correspond to
   * comparisons
   */
  ...
  /**
   * Indicates for each factor whether its join can be removed because it is the dimension table in
   * a semijoin. If it can be, the entry indicates the factor id of the fact table (corresponding to
   * the dimension table) in the semijoin that allows the factor to be removed. If the factor cannot
   * be removed, the entry corresponding to the factor is null.
   */
  ...
  /**
   * Set of null-generating factors whose corresponding outer join can be removed from the query
   * plan
   */
  Set<Integer> removableOuterJoinFactors;

  /**
   * Map consisting of all pairs of self-joins where the self-join can be removed because the join
   * between the identical factors is an equality join on the same set of unique keys. The map is
   * keyed by either factor in the self join.
   */
   ...
   * @return bitmap corresponding to the factors referenced within the specified join filter
   */
   ...
   * @return bitmap corresponding to the factors referenced by the specified factor in the various
   * join filters that correspond to comparisons
   */
  ...
  /**
   * @param factIdx factor for which information will be returned
   * @return whether or not the factor corresponds to a null-generating factor in a left or right
   * outer join
   */
  ...
  /**
   * @param factIdx factor for which information will be returned
   * @return bitmap containing the factors that a null generating factor is dependent upon, if the
   * factor is null generating in a left or right outer join; otherwise null is returned
   */
  ...
  /**
   * @param factIdx factor for which information will be returned
   * @return outer join conditions associated with the specified null generating factor
   */
   ...
   * @return the factor id of the fact table corresponding to a dimension table in a semijoin, in
   * the case where the join with the dimension table can be removed
   */
  ...
  /**
   * @param dimIdx the dimension factor for which information will be returned
   * @return the semijoin that allows the join of a dimension table to be removed
   */
  ...
  /**
   * Indicates that a dimension factor's join can be removed because of a semijoin with a fact
   * table.
   *
   ...
   * @param setFields  if true, add the fields referenced by the join filter into a map
  ...
  /**
   * Sets the bitmap indicating which factors a filter references based on which fields it
   * references
   *
   * @param fieldRefBitmap bitmap representing fields referenced
   * @return bitmap representing factors referenced that will be set by this method
   */
  ...
  /**
   * Sets weighting for each combination of factors, depending on which join filters reference which
   * factors. Greater weight is given to equality conditions. Also, sets bitmaps indicating which
   * factors are referenced by each factor within join filters that are comparisons.
   */
    ...
    for (RexNode joinFilter : allJoinFilters) {
      ...
      for (int factor : factorRefs) {
        factorsRefByFactor[factor] =
            factorsRefByFactor[factor].rebuild().addAll(factorRefs).clear(factor).build();
      }
  ...
  public List<RelDataTypeField> getJoinFields(LoptJoinTree left, LoptJoinTree right) {
    RelDataType rowType =
        factory.createJoinType(left.getJoinTree().getRowType(), right.getJoinTree().getRowType());
  ...
  /**
   * Adds a join factor to the set of factors that can be removed because the factor is the null
   * generating factor in an outer join, its join keys are unique, and the factor is not projected
   * in the query
   *
   ...
   * @return true if the factor corresponds to the null generating factor in an outer join that can
   * be removed
   */
  ...
  /**
   * Returns the other factor in a self-join pair if the factor passed in is a factor in a removable
   * self-join; otherwise, returns null.
   *
  ...
  /**
   * Determines whether there is a mapping from a column in the right factor of a self-join to a
   * column from the left factor. Assumes that the right factor is a part of a self-join.
   *
   * @param rightFactor the index of the right factor
   * @param rightOffset the column offset of the right factor
   * @return the offset of the corresponding column in the left factor, if such a column mapping
  ...
  public Edge createEdge(RexNode condition, JoinConditionSplitRes resolvedCondition,
      JoinRelType type) {
    ImmutableBitSet fieldRefBitmap = fieldBitmap(condition);
    ImmutableBitSet factorRefBitmap = factorBitmap(fieldRefBitmap);
    return new Edge(condition, resolvedCondition, factorRefBitmap, fieldRefBitmap, type);
  }

  /**
   * Create cross product edge without conditions.
   *
   * @param factorRefBitmap factor references. In case of cartesian join, the factor references can
   *                        be any two.
  ...
  public Edge createEdge(ImmutableBitSet factorRefBitmap, JoinRelType type) {
    ...
    JoinConditionSplitRes resolvedJoinConditions =
        new JoinConditionSplitRes(new ArrayList(), new ArrayList(), new ArrayList(),
            new ArrayList());
    return new Edge(truth, resolvedJoinConditions, factorRefBitmap, ImmutableBitSet.of(), type);
  }

  /**
   * Information about a join-condition. When resolving the join condition, the smaller factor is as
   * the left RelNode while the bigger factor is as the right RelNode.
   */
  static class Edge {
    ...
    Edge(RexNode condition, JoinConditionSplitRes resolvedCondition, ImmutableBitSet factors,
        ImmutableBitSet columns, JoinRelType joinType) {
    ...
    @Override public String toString() {
      return "Edge(condition: " + condition + ", factors: " + factors + ", type: " + joinType
          + ", columns: " + columns + ")";
    }
  }
  ...
  /**
   * Utility class used to keep track of the factors in a removable self-join. The right factor in
   * the self-join is the one that will be removed.
   */
  private static class RemovableSelfJoin {
    ...
    /**
     * A mapping that maps references to columns from the right factor to columns in the left
     * factor, if the column is referenced in both factors
     */
