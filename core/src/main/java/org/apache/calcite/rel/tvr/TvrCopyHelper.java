...
import org.apache.calcite.plan.volcano.TopoOrderQueue;
...
public class TvrCopyHelper {
  ...
  private final Map<RelNode, SetMultimap<TvrMetaSetType, ImmutableList<TvrEquivSemanticsSet>>>
      patterns;

  // the useless nodes that do not need to be copied
  private static Set<Class> blackList = ImmutableSet.of(MultiJoin.class);
  ...
  TvrCopyHelper(TvrVolcanoPlanner planner, ProgressiveMetrics metrics) {
    ...
    planner.getRelList().stream().filter(rel -> !(rel instanceof AbstractConverter))
    ...
    // infer missing tvrs
    this.noTvrs = new IdentityHashMap();
    metrics.startInferMissingTvr();
    initLocalMemo();
    metrics.endInferMissingTvr();

    // analyze all patterns
    this.patterns = new IdentityHashMap();
  ...
  private IdentityHashMap<RelNode, Integer> topoOrder() {
    SetMultimap<RelNode, RelNode> deps =
    ...
    IdentityHashMap<RelNode, Integer> order = new IdentityHashMap();
    while (!queue.isEmpty()) {
  ...
  class InferHelper {
    ...
    InferHelper(RelNode parent) {
      ...
      cTvrLinks = parent.getInputs().stream().map(s -> getTvrLinks(planner.getSet(s)))
      ...
      if (i == cTvrLinks.size()) {
        List<TvrDummySemantics> dedup = candidates.size() == 1
            ...
            : candidates.stream().distinct().sorted(
                Comparator.<TvrDummySemantics, TvrVersion>comparing(s -> s.fromVersion)
                    .thenComparing(s -> s.toVersion)).collect(Collectors.toList());
        switch (dedup.size()) {
        case 0:
          // for values
          break;
        ...
        case 2:
          if (parent instanceof MultiJoin) {
            ...
            if (pTvrLinks
                .put(new TvrDummySemantics(newSemantics.fromVersion, newSemantics.toVersion),
                    new TvrDummyMetaSet(tvrType))) {
              changed = true;
            }
            break;
  ...
  class AnalyzeHelper {
    ...
    private final LinkedList<SetMultimap<TvrEquivSemanticsSet, TvrMetaSet>> preprocessingTvrs;
    ...
    /**
     * Integrate tvr links (group by TvrMetaSet and TvrVersions) to reduce the number of tvr
     * matching, and all the equivalent tvr links will be saved in a TvrEquivSemanticsSet.
     *
     * For example, there is a multi-join with N inputs and it may take 2^(N+1) attempts to traverse
     * all valid matches in this case.
     *
     * MultiJoin         MultiJoin.links = [Set Snapshot MIN-8, Set Delta+ MIN-8] /\ \ \ \ R1 R2 ...
     * RN        Ri.links = [Set Snapshot MIN-8, Set Delta+ MIN-8]
     *
     * But if the tvr links have been integrated in the beginning, it only takes one attempt to
     * complete the match.
     *
     * MultiJoin         MultiJoin.links = [TvrEquivSemanticsSet MIN-8] /\ \ \ \ R1 R2 ... RN
     * Ri.links = [TvrEquivSemanticsSet MIN-8]
     */
    private void preprocessing() {
      Stream.concat(cTvrLinks.stream(), Stream.of(pTvrLinks)).forEach(links -> {
        SetMultimap<TvrEquivSemanticsSet, TvrMetaSet> setMultimap =
            Multimaps.newSetMultimap(new IdentityHashMap(), Sets::newIdentityHashSet);

        Map<TvrMetaSet, Map<Pair<TvrVersion, TvrVersion>, Set<TvrSemantics>>> m = new HashMap<>();
        links.entries().forEach(e -> {
          ...
          TvrSemantics semantics = e.getKey();
          Set<TvrSemantics> s = innerMap
              .computeIfAbsent(Pair.of(semantics.fromVersion, semantics.toVersion),
                  k -> new HashSet<>());
          s.add(semantics);
        });

        m.forEach(
            (metaSet, map) -> map.forEach((versionPair, tvrs) -> {
              setMultimap
                  .put(new TvrEquivSemanticsSet(versionPair.left, versionPair.right, tvrs, metaSet),
                      metaSet);
            }));
    ...
    private void match(int i, TvrMetaSetType tvrType) {
      if (i == preprocessingTvrs.size() - 1) {
        ...
        SetMultimap<TvrMetaSetType, ImmutableList<TvrEquivSemanticsSet>> map = patterns.get(parent);
        if (map == null) {
          map = Multimaps.newSetMultimap(new HashMap(), HashSet::new);
        ...
        for (Map.Entry<TvrEquivSemanticsSet, TvrMetaSet> e : preprocessingTvrs.get(i).entries()) {
          ...
          if (i != 0) {
            ...
            if (!versions.contains(semantics.fromVersion) || !versions
                .contains(semantics.toVersion)) {
              continue;
            }
          } else {
      ...
      for (Map.Entry<TvrEquivSemanticsSet, TvrMetaSet> e : preprocessingTvrs.get(i).entries()) {
        ...
        TvrEquivSemanticsSet semantics = e.getKey();
        if (semantics.toVersion == MAX || !candidates.stream()
            .allMatch(semantics::timeRangeOverlaps)) {
          continue;
        }
        candidates.addLast(semantics);
        match(i + 1, tvrType);
  ...
  public void kickOff() {
    ...
    while (!toCopy.isEmpty()) {
      ...
      RelNodeCopyHelper helper = new RelNodeCopyHelper(event.tvrMetaSetType, event.parent);
      helper.copy();
    }
    metrics.endCopy();
  }
    ...
    private void match(int i) {
      ...
      if (i == pattern.size() - 1) {
        ...
        for (TvrSemantics nSemantics : oSemantics.copy(from, to).equivTvrs) {
          RelSubset equivSubset = tvr.getActiveTvr().getSubset(nSemantics, parent.getTraitSet());
          final RelSubset nSubset =
              Objects.requireNonNull(planner.ensureRegistered(nParent, equivSubset));
          ...
          if (registerTvrLink(nSubset, nSemantics, tvr.getActiveTvr())) {
            ...
            triggerCandidates = planner.getSet(subset).getSubsets().stream()
                .flatMap(s -> s.getParentRels().stream()).collect(Collectors.toSet());
...
class TvrEquivSemanticsSet {
  ...
  TvrEquivSemanticsSet(TvrVersion fromVersion, TvrVersion toVersion, Set<TvrSemantics> equivTvrs,
      TvrMetaSet tvrMetaSet) {
  ...
  RelSubset getRelSubset(RelTraitSet traitSet) {
    ...
    List<RelSubset> subsets =
        equivTvrs.stream().map(t -> tvrMetaSet.getSubset(t, traitSet)).filter(Objects::nonNull)
            .collect(Collectors.toList());
  ...
  public TvrEquivSemanticsSet copy(TvrVersion from, TvrVersion to) {
    Set<TvrSemantics> newEquivTvrs =
        equivTvrs.stream().map(t -> t.copy(from, to)).collect(Collectors.toSet());
  ...
  @Override public boolean equals(Object o) {
    ...
    return Objects.equals(fromVersion, that.fromVersion) && Objects
        .equals(toVersion, that.toVersion) && Objects.equals(equivTvrs, that.equivTvrs) && Objects
...
class TvrDummySemantics extends TvrSemantics {
  ...
  @Override public TvrSemantics copy(TvrVersion fromVersion, TvrVersion toVersion) {
