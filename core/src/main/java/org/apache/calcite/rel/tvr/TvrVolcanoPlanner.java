...
import org.apache.calcite.plan.volcano.MqcOptimizer;
import org.apache.calcite.plan.volcano.MultiQueryCostOptimizer;
import org.apache.calcite.plan.volcano.ProgrExecPlanIterator;
import org.apache.calcite.plan.volcano.ProgressiveExperimentUtil;
import org.apache.calcite.plan.volcano.RelSet;
import org.apache.calcite.plan.volcano.RelSubset;
import org.apache.calcite.plan.volcano.ReuseCandidate;
import org.apache.calcite.plan.volcano.SubsetReuseStatsCollector;
import org.apache.calcite.plan.volcano.TRelOptCost;
import org.apache.calcite.plan.volcano.TvrMetaSet;
import org.apache.calcite.plan.volcano.TvrMetaSetType;
import org.apache.calcite.plan.volcano.VolcanoMemoTimeVisitor;
...
import org.apache.calcite.plan.volcano.VolcanoPlannerUtil;
import org.apache.calcite.plan.volcano.VolcanoRuleMatchLogger;
...
import static org.apache.calcite.rel.tvr.utils.TvrUtils.collectSinks;
import static org.apache.calcite.rel.tvr.utils.TvrUtils.progressiveEnabled;
import static org.apache.calcite.rel.tvr.utils.TvrUtils.progressiveMatAcrossTvrEnabled;
import static org.apache.calcite.rel.tvr.utils.TvrUtils.progressivePrintJoins;
import static org.apache.calcite.rel.tvr.utils.TvrUtils.progressiveTranslationSymmetryEnabled;
...
public class TvrVolcanoPlanner extends VolcanoPlanner {
  ...
  @Override protected VolcanoPlannerPhaseRuleMappingInitializer getPhaseRuleMappingInitializer() {
    return phaseRuleMapping -> {

      List<String> nonTvrRules = TvrRuleCollection.tvrStandardRuleSet().stream().filter(
  ...
  @Override public RelNode findBestExp() {
    ...
    if (inputTvrToSinks == null) {
      ...
      allSets.stream().flatMap(set -> set.getRelsFromAllSubsets().stream())
          ...
          .forEach(sink -> {
            TvrMetaSet inputTvr = getSet(sink.getInput(0)).getTvrForTvrSet(ctx.getDefaultTvrType());
  ...
  private RelNode runMqc(TvrContext ctx, ProgressiveMetrics pm,
    ...
    if (optMatSubsets != null) {
      ...
      reuseCollector.addPruningPredicate(candPrunePred)
          ...
          .addPruningPredicate(
              e -> e.getKey().getTvrLinks() != null && !e.getKey().getTvrLinks().isEmpty());
    ...
    while ((poll = heap.poll()) != null) {
      ...
      MatType matType =
          TvrUtils.progressiveCacheEnabled(ctx) ? MatType.MAT_MEMORY : MatType.MAT_DISK;
      matType =
...
//    queryContext.addTvrCost(tvrCost);
  ...
  private Pair<Map<RelSubset, MatType>, Map<RelSubset, MatType>> getMatSubsetFromConf(
    ...
    Map<RelSubset, MatType> forceMap = null;
    Map<RelSubset, MatType> optMap = null;
...
//        }));
    ...
    if (addNew || addNewUpstream || ctx.isDownstream()) {
      ...
      this.allSets.stream()
          .flatMap(set -> set.getTvrLinks().values().stream().map(TvrMetaSet::getTvrType))
          .distinct().forEach(tvrMetaSetType -> {
  ...
  @Override protected void postPhase(VolcanoPlannerPhase phase) {
    TvrContext ctx = context.unwrap(TvrContext.class);
  ...
  @Override protected void prePhase(VolcanoPlannerPhase phase) {
