...
import static org.apache.calcite.rel.tvr.rules.dbtoaster.GooJoinOrderingRule.JoinVertex;
import static org.apache.calcite.rel.tvr.rules.dbtoaster.GooJoinOrderingRule.LeafVertex;
...
public class CostFunctions {
  ...
  public static final CostFunction SELECTIVITY = (vertices, relNodes, multiJoin, edge) -> {
    ...
    return selectivity(vertices, factor0, factor1, relNodes, multiJoin, edge.resolvedCondition,
  ...
  public static final CostFunction CARDINALITY = (vertices, relNodes, multiJoin, edge) -> {
    ...
    return left.cost * right.cost * punishment * selectivity(vertices, factor0, factor1, relNodes,
  ...
  /**
   * If there exits NDV, the selectivity will be 1/max(NDV); otherwise, the selectivity will be
   * guessed by the filters.
   *
   * @param vertices
   * @param factor0
   * @param factor1
   ...
   * @return
   */
  public static double selectivity(List<Vertex> vertices, int factor0, int factor1,
      List<Pair<RelNode, TargetMapping>> relNodes, LoptMultiJoin multiJoin,
      JoinConditionSplitRes resolvedCondition, RexNode condition, boolean throwsIfNdvNotExist) {
    ...
    double maxNDV =
        getMaxNDV(leftVertex, rightVertex, leftPair, rightPair, resolvedCondition, multiJoin, mq);
    ...
    if (maxNDV < 0) {
      ...
      double selectivity =
          estimateSelectivity(RexUtil.composeConjunction(rexBuilder, filters, false), null, null);
      return selectivity / maxNDV;
    }
  }

  public static Double estimateSelectivity(RexNode predicate, RelNode input, RelMetadataQuery mq) {
    return 0.15;
  }
   ...
   * @return
   */
  private static ImmutableBitSet shuttleMask(ImmutableBitSet originalMask, TargetMapping mapping) {
  ...
  public static class UpdateLastCostFunction implements CostFunction {
    ...
    @Override public double cost(List<Vertex> vertexes, List<Pair<RelNode, TargetMapping>> relNodes,
    ...
    @Override public void setUpdateInputs(ImmutableBitSet updateInputs) {
