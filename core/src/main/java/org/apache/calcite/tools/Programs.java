...
public class Programs {
  ...
  public static class TvrRuleSetProgram implements Program {
    ...
    @Override public RelNode run(RelOptPlanner planner, RelNode root,
      ...
      root = tvrVolcanoPlanner
          .getOrCreateSetFromTvrSet(root, tvrType, TvrSemantics.SET_SNAPSHOT_MAX, root.getRowType(),
              root.getTraitSet());
    ...
    public RelNode addRequireOutputView(RelOptPlanner planner, RelNode root) {
      ...
      for (RelNode sink : sinks) {
        ...
        for (TvrVersion earlySnapshot : earlySnapshots) {
          RelNode sinkInputEarlySnapshot = volcanoPlanner.getOrCreateSetForTvr(root.getCluster(),
