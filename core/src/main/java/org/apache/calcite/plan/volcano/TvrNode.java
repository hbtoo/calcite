...
/**
 * Abstraction of an TvrMetaSet for de/serialization. Used to capture the structural graph of
 * related tvrs and materialized tables related to the original query output table. Downstream query
 * can then deserialize and reconstruct the graph for its tableScan.
 */
public class TvrNode implements Serializable {
  ...
  public static class TvrGraphConstructor extends TvrVisitor {
    ...
    TvrNode buildTvrGraph() {
      ...
      LOGGER.info("{} out of {} subsets reached from root tvr {}", targetSubsetsFound.size(),
          targetSubsets.size(), rootTvr.getTvrId());
    ...
    @Override public void visit(TvrMetaSet tvr, TvrProperty propertyEdge, TvrMetaSet parent) {
      if (visited.containsKey(tvr)) {
        visited.get(parent)
            .addPropertyLink(TvrPropertyUtil.toString(propertyEdge), visited.get(tvr));
        return;
      }
      ...
      if (parent != null) {
        visited.get(parent).addPropertyLink(TvrPropertyUtil.toString(propertyEdge), tvrNode);
      }

      // If it is the empty values tvr, safe this info
      if (tvr.getRelSet(TvrSemantics.SET_SNAPSHOT_MAX).getRelsFromAllSubsets().stream()
          .anyMatch(TvrUtils::isEmpty)) {
        tvrNode.isEmptyValuesTvr = true;
        tvrsToKeep.add(tvrNode);
      }

      tvr.tvrSets().forEach(
          (tvrKey, set) -> set.getSubsets().forEach(subset -> {
            if (targetSubsets.containsKey(subset)) {
              subsetToTvrNodes.computeIfAbsent(subset, x -> ArrayListMultimap.create()).
                  put(tvrKey, tvrNode);
    ...
    private void buildAllSinks() {
      subsetToTvrNodes.forEach((subset, tvrLinks) -> {
        ...
        List<TvrTableUtils.TvrTableRelation> relations = p.left.stream().map(sink -> {
          ...
          // Thus, table B needs to be created here (table A has been created by the
          // CheapestPlanBuilder),
          // and then A will be linked to B.
          if (inputSink == null) {
            EnumerableValues values = EnumerableValues
                .create(srcSubset.getCluster(), srcSubset.getRowType(), ImmutableList.of());

            String tableName = getStateTableName(ctx, srcSubset.getId(), p.right) + "_d";
            String projectName = getProjectName(ctx);
            inputSink = buildSinkForDownStream(values, oriSink, p.right, srcSubset.getTraitSet(),
                projectName, tableName,
                Objects.requireNonNull(TvrTableUtils.getEstimatedRowCount(sink.getTable())));
        ...
        RelDataType tableSchema =
            TvrJsonUtils.deserializeRowType(typeFactory, relations.get(0).tableRowType);
    ...
    private void reversePropTvrsToKeep(TvrNode node) {
      ...
      node.reverseTvrPropertyLinks.values().stream().flatMap(Collection::stream).distinct()
    ...
    private void trimGraph() {
      visited.values().stream().filter(node -> tvrsToKeep.contains(node)).forEach(node -> {
        node.tvrPropertyLinks.entrySet().removeIf(entry -> !tvrsToKeep.contains(entry.getValue()));
  ...
  private void addPropertyLink(String propertyStr, TvrNode toTvr) {
    ...
    toTvr.reverseTvrPropertyLinks.computeIfAbsent(propertyStr, x -> new HashSet<>()).add(this);
  }
  ...
  private void printGraph(Set<TvrNode> visited) {
    ...
    if (setSnapshotMaxTable != null) {
      System.out.println(
          "  " + TvrSemantics.SET_SNAPSHOT_MAX + " -> " + setSnapshotMaxTable.stream()
              .map(t -> t.tableName).reduce((s1, s2) -> s1 + "," + s2).orElse("null"));
    }
    tables.entries().forEach(
        e -> System.out.println(
        "  " + e.getKey() + " -> " + e.getValue().stream().map(t -> t.tableName)
            .reduce((s1, s2) -> s1 + "," + s2).orElse("null")));
    tvrPropertyLinks
        .forEach((property, node) -> System.out.println("  " + property + " -> tvr" + node.id));
