...
class PriorityQueue<E> {
  ...
  /**
   * Creates a {@code PriorityQueue} with the default initial capacity and whose elements are
   ...
   *                   null}, the {@linkplain Comparable natural ordering} of the elements will be
   *                   used.
   * @since 1.8
   */
  ...
  /**
   * Creates a {@code PriorityQueue} with the specified initial capacity that orders its elements
   ...
   *                              currently in this priority queue according to the priority queue's
   *                              ordering
   ...
   * (i.e., the array has more elements than the queue), the element in the array immediately
   ...
   * The following code can be used to dump the queue into a newly allocated array of {@code
   * String}:
   *
  ...
  @SuppressWarnings("unchecked") public E poll() {
    if (size == 0) {
      return null;
    }
    int s = --size;
   ...
   */
  @SuppressWarnings("unchecked") protected void siftUp(int k, E x) {
    while (k > 0) {
      int parent = (k - 1) >>> 1;
   ...
   * @param x the item to insert
   */
  @SuppressWarnings("unchecked") protected void siftDown(int k, E x) {
    int half = size >>> 1;
    while (k < half) {
      int child = (k << 1) + 1;
