...
public class PlannerMetricsListener implements RelOptListener {
  ...
  private final Map<String, AttemptMetrics> attempts = new HashMap();
  private Map<String, AttemptMetrics> lastAttempts = new HashMap();
  private final Set<String> visitedPath = new HashSet();
  ...
  @Override public void ruleAttempted(RuleAttemptedEvent event) {
    if (event.isBefore()) {
      ...
      AttemptMetrics m = attempts.computeIfAbsent(rule.toString(), k -> new AttemptMetrics());
      m.incr(l);
    }
  }
  ...
  @Override public void relChosen(RelChosenEvent event) {
    ...
    if (event.getSource() instanceof VolcanoPlanner && !visitedPath.contains(sig)) {
  ...
  public String report() {
    ...
    sb.append(String
        .format(Locale.ROOT, "%-40s%20s%20s%n", "Rule attempts (*: critical):", "Attempts",
            "Time(us)"));
    attempts.forEach((rule, m) -> {
      sb.append(
          String.format(Locale.ROOT, "%-40s%20d%20s%n", (m.isCritical ? "* " : "") + rule,
          m.getNumAttempts(), NumberFormat.getNumberInstance(Locale.US).format(m.getTimeMicros())));
      //      ThreadCounter.add(ThreadCounter.RULE_APPLY_TOTAL, rule, m.getNumAttempts(), m
      //      .getTimeMicros());
      //      if (monitor != null) {
    ...
    if (!lastAttempts.isEmpty()) {
      sb.append(System.lineSeparator()).append("delta:").append(System.lineSeparator());
      sb.append(String
          .format(Locale.ROOT, "%-40s%20s%20s%n", "Rule attempts (*: critical):", "Attempts",
              "Time(us)"));
      sb.append(System.lineSeparator());
      attempts.forEach((rule, m) -> {
        ...
        int numLastAttempts =
            lastAttempts.containsKey(rule) ? lastAttempts.get(rule).numAttempts : 0;
        long timeLastAttempts =
            lastAttempts.containsKey(rule) ? lastAttempts.get(rule).timeMicros : 0L;
        sb.append(
            String.format(Locale.ROOT, "%-40s%20d%20s%n", (m.isCritical ? "* " : "") + rule,
            m.getNumAttempts() - numLastAttempts, NumberFormat.getNumberInstance(Locale.US)
