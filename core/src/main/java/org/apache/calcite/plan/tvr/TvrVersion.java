...
public class TvrVersion implements Comparable<TvrVersion>, Serializable {
  ...
  private static final ThreadLocal<ListMultimap<Long, TvrVersion>> CACHE =
  ...
  public static final TvrVersion MIN = new TvrVersion(new long[] { MIN_TIME }) {
    @Override public boolean isMax() {
      return false;
    }

    @Override public boolean isMin() {
      return true;
    }

    @Override public long get(int tableOrd) {
      return MIN_TIME;
    }
  };
  ...
  @Override public int compareTo(TvrVersion o) {
    if (this == MAX && o != MAX) {
      return 1;
