...
public class ExecPlanManager {
  ...
  ExecPlanManager(int minTime, int lastTime, boolean reoptimize,
    ...
    this.plans = ImmutableList.copyOf(
        Stream.generate(() -> Collections.<RelNode>newSetFromMap(new IdentityLinkedHashMap<>()))
  ...
  void addHardLinkPlan(int time, PhysicalTableSink targetSink, Set<PhysicalTableSink> srcSinks) {
    ...
    if (isMultiInsert) {
      ...
      // it is better to use the plan of the linked sink directly rather than reusing the table
      // scan.
      ...
      RelNode input = inputs.size() == 1
          ? inputs.get(0)
          : new EnumerableUnion(inputs.get(0).getCluster(), inputs.get(0).getTraitSet(), inputs,
              true);
